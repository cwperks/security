...
public class Migrater {
    ...
    public static void main(final String[] args) {
        ...
        options.addOption(
            Option.builder("dir").argName("directory").hasArg().required().desc("Directory containing file to be migrated").build()
        );
    ...
    public static boolean migrateFile(File file, CType cType, boolean backup) {
        ...
        try {
            if (cType == CType.ACTIONGROUPS) {
                ...
                try {
                    val = Migration.migrateActionGroups(
                        SecurityDynamicConfiguration.fromNode(DefaultObjectMapper.YAML_MAPPER.readTree(file), CType.ACTIONGROUPS, 0, 0, 0)
                    );
                } catch (Exception e) {
                    val = Migration.migrateActionGroups(
                        SecurityDynamicConfiguration.fromNode(DefaultObjectMapper.YAML_MAPPER.readTree(file), CType.ACTIONGROUPS, 1, 0, 0)
                    );
                }
