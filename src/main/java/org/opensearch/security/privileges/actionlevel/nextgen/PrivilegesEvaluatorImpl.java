 ...
 *             <li>"config.dynamic.do_not_fail_on_forbidden": Reduction of indices is always performed when possible (however see config.dynamic.privileges_evaluation_ignore_unauthorized_indices)</li>
 ...
 *             <li>"config.dynamic.privileges_evaluation_ignore_unauthorized_indices": This new option provides a possibility to disable index reduction (formerly known the do_not_fail_on_forbidden_mode). It should be rarely necessary to use this flag. If this flag is used, users must make sure that index operations NEVER touch unauthorized indices (incl. system indices, etc).</li>
 *         </ul>
 *     </li>
 * </ul>
 */
public class PrivilegesEvaluatorImpl implements org.opensearch.security.privileges.PrivilegesEvaluator {
    private static final Logger log = LogManager.getLogger(PrivilegesEvaluatorImpl.class);
    ...
    public PrivilegesEvaluatorImpl(CoreDependencies coreDependencies, DynamicDependencies dynamicDependencies) {
        this.indexNameExpressionResolver = coreDependencies.indexNameExpressionResolver();
        ...
        this.dashboardsMultitenancySystemIndexHandler = new DashboardsMultitenancySystemIndexHandler(
            coreDependencies.clusterStateSupplier(),
            coreDependencies.client(),
            coreDependencies.threadContext(),
    ...
    PrivilegesEvaluatorResponse checkClusterPermission(PrivilegesEvaluationContext context, String action, ActionRequest request) {
        ...
        PrivilegesEvaluatorResponse dashboardsMultitenancyResponse = dashboardsMultitenancySystemIndexHandler.handle(
            request,
            action,
            context.getUser(),
            context
        ...
        if (dashboardsMultitenancyResponse != null) {
            return dashboardsMultitenancyResponse;
        }
    ...
    PrivilegesEvaluatorResponse checkIndexPermission(PrivilegesEvaluationContext context, String action, ActionRequest request) {
        ...
        PrivilegesEvaluatorResponse result = dashboardsMultitenancySystemIndexHandler.handle(request, action, context.getUser(), context);
    ...
    public boolean notFailOnForbiddenEnabled() {
        return this.indexReductionEnabled;
    }
                ...
                if (reason == null) {
                    log.debug("""
                        Allowing {} action because all privileges are present.
                          ...
                          Resolved indices: {}
                          User: {}
                        """, privilegeType, context.getAction(), requestInfo, context.getResolvedIndices(), context.getUser());
    ...
    RuntimeOptimizedActionPrivileges.SpecialIndexProtection.IndicesNeedingSpecialRoles indicesNeedingSpecialRoles(Settings settings) {
