...
public class ConfigModelV6 extends ConfigModel {
        ...
        for (Entry<String, RoleV6> securityRole : settings.getCEntries().entrySet()) {
            ...
            Future<SecurityRole> future = execs.submit(new Callable<SecurityRole>() {
                ...
                public SecurityRole call() throws Exception {
                    ...
                    // final Map<String, DynamicConfiguration> permittedAliasesIndices =
                    // securityRoleSettings.getGroups(DotPath.of("indices"));
    ...
    public static class SecurityRole {
        ...
        public boolean equals(Object obj) {
            ...
            if (tenants == null) {
                if (other.tenants != null) return false;
            } else if (!tenants.equals(other.tenants)) return false;
            return true;
        }
    ...
    public static class IndexPattern {
        ...
        public boolean equals(Object obj) {
            ...
            if (typePerms == null) {
                if (other.typePerms != null) return false;
            } else if (!typePerms.equals(other.typePerms)) return false;
            return true;
        }
        ...
        private Set<String> getResolvedIndexPattern(User user, IndexNameExpressionResolver resolver, ClusterService cs) {
            ...
            if (resolved == null || resolved.length == 0) {
                ...
                return ImmutableSet.<String>builder().addAll(Arrays.asList(resolved)).add(unresolved).build();
            }
        }
    ...
    private static boolean impliesTypePerm(
        ...
        return resolved.getAllIndices()
            .stream()
            .allMatch(
                index -> resolved.getTypes()
                    .stream()
                    .allMatch(
                        type -> Arrays.stream(requestedActions)
                            .allMatch(
                                action -> Arrays.stream(indexMatcherAndTypePermissions)
                                    .anyMatch(ipatp -> ipatp.matches(index, type, action))
                            )
                    )
            );
    }
