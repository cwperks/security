...
public class SslSettingsManagerTests extends OpenSearchTestCase {
    ...
    private Settings.Builder httpSslSettings() {
        return Settings.builder()
            ...
            .put("plugins.security.ssl.http.pemkey_filepath", path("access_http_certificate_pk.pem"));
    }
    ...
    @BeforeClass
    public static void setUpSslSettingsManagerTests() throws Exception {
    ...
    public void loadConfigurationAndBuildSslContextForSslOnlyMode() {
        ...
        assertTrue("Loaded HTTP configuration", sslSettingsManager.sslConfiguration(CertType.HTTP).isPresent());
        assertTrue("Loaded AUX FOO configuration", sslSettingsManager.sslConfiguration(MOCK_AUX_CERT_TYPE_FOO).isPresent());
        if (transportEnabled) {
            assertTrue("Loaded Transport configuration", sslSettingsManager.sslConfiguration(CertType.TRANSPORT).isPresent());
            assertTrue("Loaded Transport Client configuration", sslSettingsManager.sslConfiguration(CertType.TRANSPORT_CLIENT).isPresent());
        } else {
            assertTrue("Didn't load Transport configuration", sslSettingsManager.sslConfiguration(CertType.TRANSPORT).isEmpty());
            assertThat(
                "Didn't load Transport Client configuration",
                sslSettingsManager.sslConfiguration(CertType.TRANSPORT_CLIENT).isEmpty()
            );
        }
        assertTrue("Built HTTP SSL Context", sslSettingsManager.sslContextHandler(CertType.HTTP).isPresent());
        assertTrue("Built AUX FOO SSL Context", sslSettingsManager.sslContextHandler(MOCK_AUX_CERT_TYPE_FOO).isPresent());
        if (transportEnabled) {
            assertTrue("Built Transport SSL Context", sslSettingsManager.sslContextHandler(CertType.TRANSPORT).isPresent());
            assertTrue("Built Client SSL Context", sslSettingsManager.sslContextHandler(CertType.TRANSPORT_CLIENT).isPresent());
        } else {
            assertTrue("Didn't build Transport SSL Context", sslSettingsManager.sslContextHandler(CertType.TRANSPORT).isEmpty());
            assertTrue("Didn't build Client SSL Context", sslSettingsManager.sslContextHandler(CertType.TRANSPORT_CLIENT).isEmpty());
        }
        assertThat(
    ...
    public void loadConfigurationAndBuildSslContextForClientNode() {
        ...
        assertTrue("Didn't load HTTP configuration", sslSettingsManager.sslConfiguration(CertType.HTTP).isEmpty());
        assertTrue("Loaded Transport configuration", sslSettingsManager.sslConfiguration(CertType.TRANSPORT).isPresent());
        assertTrue("Loaded Transport Client configuration", sslSettingsManager.sslConfiguration(CertType.TRANSPORT_CLIENT).isPresent());
        assertTrue("Didn't build HTTP SSL Context", sslSettingsManager.sslContextHandler(CertType.HTTP).isEmpty());
        assertTrue("Built Transport SSL Context", sslSettingsManager.sslContextHandler(CertType.TRANSPORT).isPresent());
        assertTrue("Built Client SSL Context", sslSettingsManager.sslContextHandler(CertType.TRANSPORT_CLIENT).isPresent());
        assertThat(
    ...
    public void loadConfigurationAndBuildSslContextsMultipleAuxTransports() {
        ...
        assertTrue("Loaded HTTP configuration", sslSettingsManager.sslConfiguration(CertType.HTTP).isPresent());
        assertTrue("Loaded AUX FOO configuration", sslSettingsManager.sslConfiguration(MOCK_AUX_CERT_TYPE_FOO).isPresent());
        assertTrue("Loaded AUX BAR configuration", sslSettingsManager.sslConfiguration(MOCK_AUX_CERT_TYPE_BAR).isPresent());
        assertTrue("Loaded Transport configuration", sslSettingsManager.sslConfiguration(CertType.TRANSPORT).isPresent());
        assertTrue("Loaded Transport Client configuration", sslSettingsManager.sslConfiguration(CertType.TRANSPORT_CLIENT).isPresent());
        assertThat(
            "Built Server SSL context for HTTP",
    ...
    public void loadConfigurationAndBuildTransportSslContext() {
        ...
        final var sslSettingsManager = new SslSettingsManager(TestEnvironment.newEnvironment(settingsBuilder.build()));
        assertTrue("Didn't load HTTP configuration", sslSettingsManager.sslConfiguration(CertType.HTTP).isEmpty());
        assertTrue("Didn't load AUX configuration", sslSettingsManager.sslConfiguration(MOCK_AUX_CERT_TYPE_FOO).isEmpty());
        assertTrue("Loaded Transport configuration", sslSettingsManager.sslConfiguration(CertType.TRANSPORT).isPresent());
        assertTrue("Loaded Transport Client configuration", sslSettingsManager.sslConfiguration(CertType.TRANSPORT_CLIENT).isPresent());
        assertThat(
        ...
        assertTrue("Built HTTP SSL Context", sslSettingsManager.sslContextHandler(CertType.HTTP).isEmpty());
        assertTrue("Built AUX SSL Context", sslSettingsManager.sslContextHandler(MOCK_AUX_CERT_TYPE_FOO).isEmpty());
        assertTrue("Built Transport SSL Context", sslSettingsManager.sslContextHandler(CertType.TRANSPORT).isPresent());
        assertTrue("Built Transport Client SSL Context", sslSettingsManager.sslContextHandler(CertType.TRANSPORT_CLIENT).isPresent());
        assertThat(
            "Built Server SSL context for Transport",
            sslSettingsManager.sslContextHandler(CertType.TRANSPORT)
                .map(SslContextHandler::sslContext)
    ...
    private static void assertThat(String message, boolean condition) {
