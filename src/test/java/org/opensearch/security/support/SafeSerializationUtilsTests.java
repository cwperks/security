...
public class SafeSerializationUtilsTests extends LuceneTestCase {
    ...
    public void testUnsafeClasses() {
        assertFalse(SafeSerializationUtils.isSafeClass(SafeSerializationUtilsTests.class));
    ...
    public void testProhibitUnsafeClasses() {
        ...
        try {
            SafeSerializationUtils.prohibitUnsafeClasses(SafeSerializationUtilsTests.class);
            fail("Should throw exception for unsafe class");
        } catch (IOException e) {
            assertEquals("Unauthorized serialization attempt " + SafeSerializationUtilsTests.class.getName(), e.getMessage());
        }
    }
